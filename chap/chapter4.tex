...
3シーズン合計の対象試合数は87試合であり，これらすべての試合について，第3章で述べた25カテゴリ（チケット種別×ファン層）の販売構成を算出し，Game2Vecの学習および第4.2章以降の評価実験に用いた．
...
また，本研究で扱う試合では，各試合で販売期間が異なるため，販売開始日を基準とした日数を共通の時間軸に整列させた．
本研究で対象とする試合はいずれも販売期間が20日を超えるため（$D>20$），販売開始からの経過日数 $d$ を
$d'\in\{0,1,\ldots,30\}$ に写像する区分的な再スケーリングを適用した．
具体的には，初動および終盤の情報を保持するため，
(1) 販売開始後10日間（$d\le 10$）は $d'=d$，
(2) 販売終了前10日間（$d\ge D-10$）は $d'=20+(d-(D-10))$ として保持し，
(3) それ以外の中間期間（$10<d<D-10$）を $d'=11$ から $20$ の範囲に線形に再スケールして割り当てた．
このとき，中間期間の写像は連続値を取り得るため，日次の時系列として扱う目的で最終的に四捨五入し，
$d'$ を整数日として離散化した．
これにより，全試合の販売推移を「初動10日＋中盤10日＋終盤10日」の同一枠組みで比較可能にした．

上記の整列後の日数 $d'$ に基づき，全試合を対象としてチケット種別の累積販売枚数を集計し，その時系列推移を図\ref{cum_price_ex}に示す．
ここでは，販売開始後の各日（$d'=0,1,\ldots,30$）における累積販売枚数を，招待・優待・有料（低価格帯／中価格帯／高価格帯）のチケット種別に算出した．
図\ref{cum_price_ex}は，評価実験で扱うデータの販売規模と，チケット種別による構成の概観を与えるものである．
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.8\linewidth]{images/ticket_cum_price_ver1.pdf}
    \caption{累積販売枚数の時系列推移}
...
さらに，チケット種別ごとの販売規模に加えて，各チケットがどのファン層によって購入されているかを把握するため，
チケット種別ごとに累積販売枚数をファン層別に分解した時系列推移を図\ref{cum_fan_ex}に示す．
図は全試合を対象に，前節で述べた販売期間の整列後の時間軸（販売開始後0〜30日）に基づいて累積販売枚数を集計し，
各チケット種別についてファン層（超ライト層〜超コア層）を積み上げ表示したものである．
図\ref{cum_fan_ex}より，招待チケットおよび優待チケットでは，超ライト層の寄与が大きく，
次いでミドル層が一定の割合を占める一方，コア層・超コア層の寄与は相対的に小さいことが分かる（図\ref{fig:1}，\ref{fig:2}）．
これは，招待・優待がライト層を中心とする来場促進施策として機能している可能性を示唆する．
有料チケットについては，低価格帯では販売期間を通じて超ライト層とライト層の寄与が大きく，
累積販売が比較的なだらかに増加する傾向が見られる（図\ref{fig:3}）．
一方，中価格帯および高価格帯では販売終盤（およそ25日目以降）に累積販売が大きく増加し，
その増分の一部はコア層・超コア層の寄与によって構成されることが確認できる（図\ref{fig:4}，\ref{fig:5}）．
このことから，価格帯によって購入主体となるファン層や購入タイミングの違いが確認される．
...
\begin{figure*}[!t]
  ...
  \begin{minipage}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/ticket_cum_syotai_price_ver2.pdf}
    \subcaption{招待チケット}
    \label{fig:1}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/ticket_cum_yutai_price_ver2.pdf}
    \subcaption{優待チケット}
    \label{fig:2}
  \end{minipage}
  ...
  \begin{minipage}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/ticket_cum_low_price_ver2.pdf}
    \subcaption{低価格チケット}
    \label{fig:3}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/ticket_cum_middle_price_ver2.pdf}
    \subcaption{中価格チケット}
    \label{fig:4}
  \end{minipage}
  ...
  \begin{minipage}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/ticket_cum_high_price_ver2.pdf}
    \subcaption{高価格チケット}
    \label{fig:5}
  \end{minipage}
  \hfill
...
ここでは，$k$を変化させたときのSSE（クラスタ内平方和）の推移を確認し（図\ref{fig:elbow_kmeans}），減少が緩やかになる点を目安として$k=7$を採用した\cite{yuan2019research}．
...
\begin{figure}[tb]
    ...
    \includegraphics[width=0.6\linewidth]{images/elbow_ver1.pdf}
    \caption{エルボー法によるSSEの推移}
...
\begin{figure}[tb]
    ...
    % \includegraphics[width=\linewidth]{imgs/game2vec_vis_final_base.pdf}
    % \includegraphics[width=\linewidth]{imgs/game2vec_vis_4days_base_ver4.pdf}
    \includegraphics[width=1.0\linewidth]{images/game2vec_vis_4days_base_ver7.pdf}
...
\begin{figure}[tb]
    \centering
    % \includegraphics[width=\linewidth]{imgs/game2vec_vis_base_final.pdf}
    \includegraphics[width=1.0\linewidth]{images/game2vec_vis_final_base_ver6.pdf}
...
販売開始から $t$ 日目までの累積販売構成を用いて学習した Game2Vec の埋め込みベクトルを $\mathbf{z}_i^{(t)} \in \mathbb{R}^3$，最終販売構成から学習した最終ベクトルを $\mathbf{z}_i^{(\mathrm{final})} \in \mathbb{R}^3$ と表す（$i=1,\dots,G$）．
それぞれのベクトル空間において，試合ペア $(m,n)$（$1 \le m < n \le G$）間のユークリッド距離


\begin{equation}
\begin{split}
  l_{mn}^{(t)} &= \left\lVert \mathbf{z}_m^{(t)} - \mathbf{z}_n^{(t)} \right\rVert_2 \\
  l_{mn}^{(\mathrm{final})} &= \left\lVert \mathbf{z}_m^{(\mathrm{final})} - \mathbf{z}_n^{(\mathrm{final})} \right\rVert_2
\end{split}
\end{equation}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\linewidth]{images/Scatter_plot_of_latent-space_similarity_ver3.pdf}
    % \includegraphics[width=0.6\linewidth]{images/ticket_cum_price_ver1.pdf}
    % \caption{試合間距離の散布図}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{images/Temporal_changes_in_the_latent_space_ver3.pdf}
...
これらはいずれも，試合開催前に取得可能であり，既存の来場予測研究においても一般的に利用されてきた説明変数である．
外的要因に含まれるカテゴリ変数は順序を持たないカテゴリとして整数エンコードし，モデルへ入力した．
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{images/model_comparison_final_ver3.pdf}
...
ここで，外的要因のみのモデルと，外的要因にGame2Vec試合ベクトルを加えたモデルについて，ランダムフォレストの特徴量重要度を図\ref{fig:fi_compare}に示す．
外的要因のみのモデルでは，収容人数（capacity）や対戦相手の勝率（opponent\_winrate）などが比較的重要度の高い特徴量となった．
一方，外的要因にGame2Vec試合ベクトルを追加したモデルにおいても，capacityやopponent\_winrateは依然として上位に位置している．
その上で，試合ベクトルも高い重要度を示しており，販売開始後の初動販売から得られる情報が外的要因を補完する形で利用されたことで，予測精度が向上したと考えられる．
ただ，Game2Vecの各次元の重要度には差が見られ，一部の次元（dim0, dim2）が相対的に高い重要度を示す．
これは、重要度の低い次元には外的要因でも説明可能な販売傾向の差が主として反映されており，重要度の高い次元には外的要因では捉えにくい最終的な販売傾向に関する情報が，初動販売から抽出されて反映された可能性がある．

\begin{figure}[t]
  \centering
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/external_feature_importance_ver1.pdf}
    \caption{外的要因のみ}
    \label{fig:fi_external_only}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/game2vec_feature_importance_ver1.pdf}
    \caption{外的要因＋Game2Vec}
    \label{fig:fi_external_game2vec}
  \end{subfigure}
  \caption{予測モデルにおける特徴量重要度の比較}
  \label{fig:fi_compare}
\end{figure}
...
図\ref{fig:prediction_final}より，VenueFでの予測精度の大幅な改善が，確認できる．
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{images/finalrate_25category_venueF_ver4.pdf}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.9\linewidth]{images/finalrate_true_25category_venueF_ver5.pdf}
...
一方で，図\ref{fig:prediction_final}より，本実験において総販売率の予測誤差が増加した唯一の会場は Venue G であった．
